...
for  %%i in (%newdata%) do (
  ...
  set  result=!x! BDD\featuresSmokeSuite\%%i.feature
	set x=!result!
  rem echo !result!
  rem echo !x!
)
...
IF %runParallel% == true (
	IF %tag% == @all (
		...
		IF %tag% == @all (
			echo "inside if"
			echo " ..\..\..\Utilities\GwenSetup\gwen-web-2.22.0\bin\gwen -b  -r reports -f "junit,json,html"  -p ".\BDD\properties\application.properties,.\BDD\properties\gwen.properties"  BDD\featuresSmokeSuite\*%2*"
			call ..\..\..\Utilities\GwenSetup\gwen-web-2.22.0\bin\gwen -b  -r reports -f "junit,json,html"  -p ".\BDD\properties\application.properties,.\BDD\properties\gwen.properties"  BDD\featuresSmokeSuite\*%2*
		) ELSE (
			echo "..\..\..\Utilities\GwenSetup\gwen-web-2.22.0\bin\gwen -b  -r reports -f "junit,json,tml"  -p ".\BDD\properties\application.properties,.\BDD\properties\gwen.properties" -t %tag% BDD\featuresSmokeSuite\*%2*"
			call ..\..\..\Utilities\GwenSetup\gwen-web-2.22.0\bin\gwen -b  -r reports -f "junit,json,html"  -p ".\BDD\properties\application.properties,.\BDD\properties\gwen.properties" -t "%tag%" BDD\featuresSmokeSuite\*%2*
		)
		set reportfolder=%2
	)
